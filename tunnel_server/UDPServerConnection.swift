...
class UDPServerConnection: Connection {
    ...
    override func sendDataWithEndPoint(data: NSData, host: String, port: Int) {
		...
		guard sent > 0 else {
			if let errorString = String(UTF8String: strerror(errno)) {
				print("UDP connection id \(identifier) failed to send data to host = \(host) port \(port). error = \(errorString)")
			}
			closeConnection(.All)
			return
		}
		...
		if sent == data.length {
			// Success
			print("UDP connection id \(identifier) sent \(data.length) bytes to host = \(host) port \(port)")
		}
    }
