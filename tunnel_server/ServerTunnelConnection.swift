...
class ServerTunnelConnection: Connection {
	...
	func open() -> Bool {
		...
		guard let address = ServerTunnel.configuration.addressPool?.allocateAddress() else {
			print("Failed to allocate a tunnel address")
		...
		guard setupVirtualInterface(address) else {
			print("Failed to set up the virtual interface")
		...
		var response = createMessagePropertiesForConnection(identifier, commandType: .OpenResult)
		...
		guard let IPv4Dictionary = personalized[SettingsKey.IPv4.rawValue] as? [NSObject: AnyObject] else {
			print("No IPv4 Settings available")
			sendOpenResult(.InternalError)
			return false
		}
	...
	func readPackets() {
		...
		let iovecListPointer = UnsafeBufferPointer<iovec>(start: &iovecList[0], count: iovecList.count)
	...
	override func sendPackets(packets: [NSData], protocols: [NSNumber]) {
		...
		for (index, packet) in packets.enumerate() {
			...
			if writeCount < 0 {
				...
				print("Wrote \(writeCount) bytes of a \(packet.length) byte packet to utun")
			}
		}
	}
}
