...
class ServerTunnel: Tunnel, TunnelDelegate, NSStreamDelegate {
	...
	func handleBytesAvailable() -> Bool {
		...
		var readBuffer = [UInt8](count: Tunnel.packetSize, repeatedValue: 0)
		...
		repeat {
			...
			if packetBytesRemaining == 0 {
				...
				if packetBuffer.length == sizeof(UInt32.self) {
					...
					guard totalLength <= Tunnel.maximumMessageSize else { return false }
			...
			case writeStream!:
				switch eventCode {
					case [.HasSpaceAvailable]:
						...
						if !savedData.isEmpty && savedData.writeToStream(writeStream!) {
    ...
    override func closeTunnel() {
        ...
        if let stream = readStream {
            if let error = stream.streamError {
                print("Tunnel read stream error: \(error)")
            }
        ...
        if let stream = writeStream {
            if let error = stream.streamError {
                print("Tunnel write stream error: \(error)")
            }
...
class ServerDelegate : NSObject, NSNetServiceDelegate {
	...
	func netService(sender: NSNetService, didNotPublish errorDict: [String : NSNumber]) {
		print("Failed to publish network service")
		exit(1)
	}

	/// Handle the "published" event.
	func netServiceDidPublish(sender: NSNetService) {
		print("Network service published successfully")
	}
